...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalMultVectors = x1 * x2 + y1 * y2;
  const lenX = Math.abs(Math.sqrt(x1 ** 2 + y1 ** 2));
  const lenY = Math.abs(Math.sqrt(x2 ** 2 + y2 ** 2));
  const result = (scalMultVectors / lenX) * lenY;
...
function getCountOfOddNumbers(number) {
  ...
  return number % 2 ? (absNumber + 1) / 2 : absNumber / 2;
}
